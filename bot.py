...
async def find_inviter(guild: discord.Guild) -> discord.User | None:
    """
    Try multiple times with increasing delay to find which invite was used.
    Needed for Community servers where Discord updates invite counts slowly.
    """
...
async def on_member_join(member: discord.Member):
    ...
    # Invite tracking — run in background so welcome message isn't delayed
    bot.loop.create_task(handle_invite(member))


async def handle_invite(member: discord.Member):
    """Find inviter with retries, then post to invite channel."""
...
@tree.command(name="level", description="Check your level or someone else's level.")
